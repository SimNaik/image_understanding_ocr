...
def create_and_copy_to_mo_folder(misclassified_annotations_per_conf, gt_target_dir, mo_source_dir, target_dir):
    """
    Creates an additional '_MO' folder and copies the corresponding files from the
    `mo_source_dir` to the target '_MO' folder for each misclassified image, 
    based only on filenames from the '_GT' folder.

    Args:
        misclassified_annotations_per_conf: Misclassified annotations for different thresholds.
        gt_target_dir: Source directory for ground truth files.
        mo_source_dir: Source directory for model outputs (predictions).
        target_dir: Directory for saving misclassified images and '_MO' folder.
    """
    ...
    # Debugging: Print contents of GT and model output directories
    print(f"Contents of GT directory: {os.listdir(gt_target_dir)}")
    print(f"Contents of Model output directory: {os.listdir(mo_source_dir)}")
    
    # Go through each confidence threshold
    for conf_threshold, misclassified_images in misclassified_annotations_per_conf.items():
        print(f"Processing confidence threshold: {conf_threshold}")
        
        # Extract image paths from misclassified annotations
        image_paths = [m['image_id'] for m in misclassified_images]
        ...
        # Process each unique image
        for image_name in unique_images:
            # Flag to check if the image exists in gt_target_dir
            found_gt_image = False
            print(f"Checking for misclassified image: {image_name}")
            
            for ext in ['.jpg', '.png']:  # Only check for .jpg and .png extensions
                # Check if the image exists in the _GT folder
                gt_image_path = os.path.join(gt_target_dir, f"{image_name}{ext}")
                print(f"Checking GT image path: {gt_image_path}")
                
                if os.path.exists(gt_image_path):
                    found_gt_image = True
                    # If the image exists in the _GT folder, find the corresponding image in the model output folder
                    mo_image_path = os.path.join(mo_source_dir, f"{image_name}{ext}")
                    print(f"Checking model output path: {mo_image_path}")
                    
                    if os.path.exists(mo_image_path):
                        # If the image exists in the model output folder, copy it to the _MO folder
                        target_image_path = os.path.join(mo_target_dir, f"{image_name}{ext}")
                        shutil.copy(mo_image_path, target_image_path)
                        print(f"Copied {image_name}{ext} from model output to _MO folder.")
                    else:
                        print(f"Model output not found for {image_name}{ext} in {mo_source_dir}.")
                    break  # Exit the extension loop once a valid file is found
            
            if not found_gt_image:
                print(f"Ground truth not found for {image_name} in {gt_target_dir}.")
